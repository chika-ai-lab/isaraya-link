...
// GET /api/commercials?profileId=... — liste des commerciaux (public)
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json(toSnake(commercials));
}

// POST /api/commercials — créer un commercial
export async function POST(request: NextRequest) {
  ...
  const commercial = await prisma.commercial.create({ data: body });
  return NextResponse.json(toSnake(commercial), { status: 201 });
}
